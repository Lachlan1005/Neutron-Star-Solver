...
c    = 3 * 10**5          # km/s
Rsun = 696340             # km
def WDrk4(f:callable, g:callable, x:float, y:float, dt:float, tmax:float, gamma, a, b, name:str="Test System", epsilon0:float=10**(-12),graph:bool=True,verbosity:bool=True)->list[list[float]]:
    """
    ...
    while isinstance(x,float) and x>=10**(-23.69):
        if verbosity:
...
def runSimulation(a:float,b:float,gamma:float,epsilon0:float, p0, dr, customAB:bool=False, A:float=1, Z:float=2,graph:bool=True, verbosity:bool=True):
    ...
    return WDrk4(pressureChange,massChange,p0,0,dr,epsilon0, gamma, a, b, "Polytrope",10**(-12),graph, verbosity)
...
def terminalSimulation():
    ...
    if preset==0:
        ...
        gamma=float(input("Enter parameter gamma (Examples include 1.25 for relativistic, 1.67 for nonrelativistic, 2.11 for nuclear): "))
        epsilon0=10**(-12)
